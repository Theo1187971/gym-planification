<template>
  <router-link to="/my-account" class="link-a">My account</router-link>
  <br><br>
      ...
      <v-dialog v-model="dialogAddExercise" max-width="600">
        <v-card>
          ...
          <v-card-actions class="d-flex justify-center" style="gap: 16px; padding-bottom: 24px;">
            <v-btn color="grey"
                   style="height: 48px; min-width: 120px; font-size: 16px;" text @click="cancelAddExercise()">Cancel
            </v-btn>
            <v-btn color="red"
                   style="height: 48px; min-width: 120px; font-size: 16px;" text @click="createExercise()">Confirm
            </v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
